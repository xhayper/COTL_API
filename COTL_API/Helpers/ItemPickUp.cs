...
{
    /// <summary>
    ///     Returns a GameObject of the specified item type (if it exists). Will return a gold coin if the item type is not
    ///     found.
    /// </summary>
    ...
    {
        LogHelper.LogWarning($"ImitatePickUpObject: {type}");
        var itemText = "";
        switch (type)
        {
        ...
        LogHelper.LogWarning($"ImitateItemPickUpText: {itemText}");
